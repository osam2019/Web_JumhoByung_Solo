		...
		.map((room, index)=>{
			return {
				...
				attend: room.filter(soldier => { 
					return soldier.morningAttend===true && soldier.status==="완료"
				}).length, 
				abscent: room.filter(soldiers => soldiers.morningAttend!==true && soldiers.status==="완료").length,
				uncompleted: room.filter(soldiers => soldiers.status==="미완료").length,
		}});
		
        return (
            <React.Fragment>
				<Title>토성 4 생활관 오전 점호 인원</Title>
                <ResponsiveContainer>
                    <BarChart
                        width={900}
                        height={300}
